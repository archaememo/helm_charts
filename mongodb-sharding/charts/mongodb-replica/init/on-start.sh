...
function shutdown_mongo() {
    ...
    mongo admin --port ${srv_port} "${admin_creds[@]}" "${ssl_args[@]}" --eval "db.shutdownServer({$args})"
}
...
mongod "$srv_type" --config /data/configdb/mongod.conf --dbpath=/data/db --replSet="$replica_set" --port="$srv_port" "${auth_args[@]}" --bind_ip=0.0.0.0 >> /work-dir/log.txt 2>&1 &
...
if mongo --port ${srv_port} "${ssl_args[@]}" --eval "rs.status()" | grep "no replset config has been received"; then
    log "Initiating a new replica set with myself ($service_name)..."
    mongo --port ${srv_port} "${ssl_args[@]}" --eval "rs.initiate({'_id': '$replica_set', 'members': [{'_id': 0, 'host': '${service_name}:${srv_port}'}]})"

    sleep 3

    log 'Waiting for replica to reach PRIMARY state...'
    until printf '.'  && [[ $(mongo --port ${srv_port} "${ssl_args[@]}" --quiet --eval "rs.status().myState") == '1' ]]; do
        sleep 1
    done
    ...
    if [[ "$AUTH" == "true" ]]; then
        ...
        mongo admin --port ${srv_port} "${ssl_args[@]}" --eval "db.createUser({user: '$admin_user', pwd: '$admin_password', roles: [{role: 'root', db: 'admin'}]})"
        if [[ "$METRICS" == "true" ]]; then
            log "Creating cluterMonitor user..."
            mongo admin --port ${srv_port} "${ssl_args[@]}" --eval "db.auth('$admin_user', '$admin_password'); db.createUser({user: '$metrics_user', pwd: '$metrics_password', roles: [{role: 'clusterMonitor', db: 'admin'}, {role: 'read', db: 'local'}]})"
        fi
    fi
